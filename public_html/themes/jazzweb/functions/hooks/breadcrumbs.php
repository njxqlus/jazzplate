...
class breadcrumbs {
    ...
    public function bitem($bitem, $link = true)
    {
        ...
        if (is_string($bitem->url) && !empty($bitem->url))
        {
            ...
            if ($link == true)
            {
                ?><a href="<?php echo $bitem->url; ?>"
                     title="<?php echo $bitem->name; ?>"><?php echo $bitem->name; ?></a><?php
            } else
            {
                echo $bitem->name;
            }
    ...
    public function get($sep = '/')
    {
        ...
        if (is_404())
        {
            $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('page404', false)]);
        ...
        } elseif (is_page())
        {
            ...
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('post', false)]);
            } else
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('parent_page'), $this->bitem('post', false)]);
            }
        } elseif (is_singular() && !is_page())
        {
            if (is_custom_post_type())
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('post_type'), $this->bitem('post', false)]);
            } else
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('category'), $this->bitem('post', false)]);
            }
        ...
        } elseif (is_archive())
        {
            if (is_tax())
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('tax', false)]);
            } elseif (is_post_type_archive())
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('post_type', false)]);
            } elseif (is_category())
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('category', false)]);
            } else
            {
                $breadcrumbs = implode($sep, [$this->bitem('main'), $this->bitem('archive', false)]);
            }
        }
